...
export function HooksPage() {
  ...
  return (
    <div className="flex flex-col min-h-full" style={{ animation: "pageIn 0.5s ease both" }}>
      {/* ── Page header ── */}
      <div
        className="flex-shrink-0 flex items-start justify-between gap-4 px-6 pt-8 pb-5 border-b"
        style={{ borderColor: "rgba(255,255,255,0.05)" }}
      >
        <div>
          <h1
            className="font-serif tracking-tight mb-1"
            style={{ fontSize: "clamp(1.5rem, 3vw, 2rem)", color: "rgba(255,255,255,0.92)" }}
          >
            Hooks
          </h1>
        ...
        <button
          ...
          className="flex-shrink-0 h-9 px-5 rounded-full text-[13px] font-semibold transition-all hover:brightness-110 active:scale-95"
      ...
      {/* ── Filter tabs ── */}
      <div
        className="flex-shrink-0 flex gap-1 px-5 py-3 overflow-x-auto border-b"
        style={{ borderColor: "rgba(255,255,255,0.05)" }}
      >
        {FILTER_TABS.map((tab) => (
          <button
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Hooks list */}
        <div
          className={`flex-1 overflow-y-auto transition-all duration-300 ${hasPanel ? "hidden lg:block lg:max-w-[55%]" : ""}`}
        >
        ...
        {hasPanel && (
          <div
            className="w-full lg:w-[45%] flex-shrink-0 border-l overflow-hidden flex flex-col"
            style={{
